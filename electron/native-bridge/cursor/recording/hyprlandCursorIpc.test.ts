...
describe("queryHyprlandCursorPos", () => {
	...
	it("resolves with null when the server accepts but never responds (timeout)", async () => {
		const socketPath = createTestSocketPath();
		// Accept the connection and read the command, but never write a response --
		// exercises the SOCKET_TIMEOUT_MS timeout path instead of "close"/"error".
		const server = net.createServer((socket) => {
			socket.once("data", () => {
