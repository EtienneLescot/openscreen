...
| --- | --- | --- | --- |
| Windows | Windows Graphics Capture (WGC) helper (C++/Win32), with WASAPI and Media Foundation support | `electron/native/wgc-capture/` and `electron/windows.ts` | H.264 MP4 screen/window video; system/microphone AAC when enabled; webcam as its own MP4 |
...
| Webcam | Native Media Foundation first, exact Electron-resolved DirectShow fallback; its own MP4 | Electron sidecar attached to the session | Electron sidecar attached to the session |
...
A session writes a screen video and a `.session.json` manifest. **The webcam is always its own file, on every platform.** Windows passes `outputs.webcamPath` unconditionally (`handlers.ts`), so the helper takes its separate-file branch; macOS and Linux attach the webcam as an Electron `MediaRecorder` sidecar (`webcamVideoPath`) because native webcam capture is not part of either helper. Audio that the selected backend captures is encoded into its screen output.

That the camera is never baked into the screen video is what makes every camera treatment in the editor — mask shape, mirroring, size, position, and the chroma key — a *setting* rather than a decision taken at record time. The Windows helper does still carry an inline PiP compositor (`mf_encoder.cpp::compositeWebcam`), reachable only when no `webcamPath` is given; nothing in the current Electron code takes that path, and reviving it would forfeit the property above. Composition happens once, later, in the Rust compositor: see [preview.md](preview.md) and [export-pipeline.md](export-pipeline.md).
