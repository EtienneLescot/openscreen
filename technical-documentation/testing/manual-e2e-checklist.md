...
- [ ] With webcam enabled, start a new recording and confirm the native floating self-view opens automatically; pause/resume must leave it open, while stop, cancel, restart teardown, camera loss, and HUD destruction must close it.
...
- [ ] Drag and resize the native self-view across all four corners, another Space, and a fullscreen app. Confirm it stays above ordinary windows. Record any placement macOS refuses as a native PiP limitation; the app does not claim programmatic positioning.
...
- [ ] Simulate webcam-track termination and a rejected/unsupported PiP request; confirm recording continues and the self-view failure is non-blocking. Physically disconnect the camera and move across displays when the hardware is available; otherwise mark those two checks skipped, not passed.
