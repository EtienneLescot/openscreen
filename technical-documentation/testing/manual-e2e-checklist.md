...
- [ ] Probe the finished file and confirm the **audio and video durations agree to within one frame**; a stretch that fell short would show as an audio track measurably shorter than the video.
- [ ] Run `silencedetect` over the exported audio and confirm no silence block coincides with a speed region's boundary.
- [ ] Confirm the exported duration matches the source minus the trimmed spans, adjusted for each speed region.
