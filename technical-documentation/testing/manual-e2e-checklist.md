...
| 2026-09-05 | installed `v1.11.0-rc.1`, macOS Apple Silicon DMG from CI build run 33958219380 (`openscreen-mac-arm64`). Developer ID signed, **notarized** (`spctl -a -t exec` accepted, `source=Notarized Developer ID`), ticket stapled to the DMG. About reports `1.11.0-rc.1 (dmg)`, Electron 41.2.1 / Chromium 146.0.7680.188 / Node 24.14.1. Native payload complete and uniformly stamped (33 files in `Resources/electron/native/bin/darwin-arm64`, all `Sep 5 11:36`: compositor addon, SCK + cursor helpers, whisper-stt-server, ffmpeg dylibs, libonnxruntime) | macOS 26.5 (25F71), M1, 1920x1080 @1x, plus a 2304x1296 / 1152x648 **@2x** pass for the HiDPI checks | **Partial - 1 blocker, 4 minor** | **Blocker: local transcription is dead in the packaged macOS build.** The helper is healthy - `[whisper-stt] model loaded; backend=whispercpp-metal`, `listening on 127.0.0.1:56198`, model reused from `stt-models/whisper-ggml/ggml-small-q8_0.bin` - but every `stt:transcribe` dies before it starts: `FfmpegUnavailableError: stt:native-extraction-unavailable: no ffmpeg binary for native audio extraction` at `SCe.extract` -> `SCe.transcribe`. The `.app` bundles the ffmpeg **shared libraries** but no ffmpeg **executable** (`find /Applications/Openscreen.app -name 'ffmpeg*'` is empty) and this machine has no system ffmpeg, which is the normal user's situation. All the user is told is **"Failed to fetch"** - the same misleading string the v1.10.0-rc.3 row flagged, now on the far more common path. Reproduced on demand from the Transcribe now button, and again from **Regenerate** in the Media panel's Source Transcript on a clean launch: `[stt] done` count is **1** in the instance with the env var and **0** without it, same build, same file, one variable. **The failure is masked by the cached transcript**: because the transcript is stored on the asset, the panel keeps showing **"Transcript ready" + "Detected language: French"** with the full text while "Failed to fetch" appears underneath — two contradictory states at once. Anyone who has ever had one successful run will conclude transcription works; a real user, with no cached transcript, sees only "Failed to fetch". **Root cause proven**: relaunching the unmodified build with only `OPENSCREEN_FFMPEG_PATH` pointed at an ffmpeg wrapper made it work end to end - `[stt] done on whispercpp-metal: 1 chunk(s), 65.5s audio in 19.3s (0.29 rtf, 3.4x real-time)`. Note the bundle already contains a fallback for this exact error string at another call site; the failing one has none. **Everything downstream is blocked on a stock install**: transcript, captions, caption placement, and transcript word editing. Those were then run against the same build with the env var set, and all passed - see below. **Three shipped regressions did NOT reproduce.** (1) **#375** (the v1.9.5 macOS blocker where a take with system audio died at ~1s and was discarded): a 65s screen recording **with system audio** finalised cleanly - 418,877,966 bytes, 1920x1080, AAC 48kHz stereo, video 65.491s / audio 65.530s, `ffmpeg -f null -` exit 0, mean_volume -12.8 dB, all three sidecars, editor opened in <10s. Clean-stop box shape is `ftyp mdat moov` with no `mvex`/`moof`/`mfra`, which the v1.9.5 row already establishes as normal macOS finalisation. (2) **the rc.3 window-capture crash** (`CGS_REQUIRE_INIT` SIGABRT): the Windows tab enumerated 6 windows, selecting one recorded fine - 1468x854 (the window, not the 1920x1080 screen), 1161 frames, both sidecars, **no crash reports**. (3) **#418** (HiDPI frame not filled): re-tested at a real 2x scale. Entire screen -> **2304x1296**, all four edges 100% non-black, all four quadrants 99.5-100% non-black; single window -> **2304x1058** (the window's pixel size), edges 90-96% non-black. Measured on the frame, not the file header. **Export is in good shape and the 2026-09-03 open question is closed.** Two MP4 exports at 1080p/30/H.264. Run A (trim 30.200045-32.284309 + 1.5x speed 35.259-37.343): predicted total 1884 frames, expected 65.530 - 2.084264 - 0.694667 = 62.751069s = 1882.53 frames, **measured 1883 frames** (`-count_frames`) = **0.47 frames** out. Run B (same trim + **2x** speed 15.024-17.108): predicted 1874, expected 62.403737s = 1872.11 frames, **measured 1872** = **0.11 frames** out. The ~13-frame shortfall logged on 2026-09-03 does not reproduce. A/V agree within one frame **at 30 fps** both times (62.733333 vs 62.766000 = 0.98 frames; 62.400000 vs 62.433000 = 0.99 frames) - right at the edge, so the rate matters. Progress reached a directly observed **100%** (1844/1847). **The atempo silence check is finally conclusive**: run A's speed region sat in a stretch where the source audio had already run out, so it proved nothing (exactly the trap the checklist warns about); moving the region into continuous -9.7 dB audio and re-exporting gives `silencedetect -45dB d=0.05` hits only at the head (0-0.088s) and from 29.199979s (the source's own silence arriving through the trim - source goes quiet at 31.970s, and 30.200 - 1.042 = 29.158), **nothing at either 15.024 or 16.066**, with level flat at -9.7/-9.7/-9.7/-9.5/-9.7 dB straight across both boundaries. **Caption placement measured, not eyeballed**: with the saved `anchorV:"top", insetY:1.5, anchorH:"left", insetX:10`, an exported 1920x1080 frame has its plate's left edge at **x=192** on every row from y=20 to y=80 (1920 x 10% exactly) and its top edge first appearing at **y=16** against 1080 x 1.5% = 16.2. The Position row offers Bottom/Top and a separate Left/Center/Right, the hint flips between "grow upward - the bottom edge stays put" and "grow downward - the top edge stays put", and the slider relabels Distance from bottom/top/left. **GIF**: 938 frames (predicted total matched), 1280x720, 136.8 MB, NETSCAPE2.0 loop extension with count 0, decode clean, and **no LZW code-width corruption** - frame ~867 of 938 on colour-rich footage is clean. ffmpeg and macOS ImageIO (Preview) agree. Both exports open in native viewers (QuickTime reads 01:02 against the measured 62.40s). **post-1.10.0 update settings pass**: the tray submenu offers exactly the three documented modes, selecting one wrote `update-settings.json` = `{"mode":"download"}` and it was still checked after a full restart; Save Diagnostics writes a bundle; Check for Updates reports "up to date (1.11.0-rc.1)". **post-1.10.0 word-insertion gate passes against the packaged build**: typing between two words did nothing (no transcript change, no amber clip), the hint does not advertise adding words, and correcting an ordinary word still works and propagates into the captions. Also passed: HUD tray layout toggle, drag (tracks the pointer, clamps at the menu bar, stays where dropped), locale menu, device panel reporting "No microphone found"/"No camera found" correctly; source selector; pause (timer froze at 00:31 across 16s, indicator red->amber, resume advanced again); play/pause/seek; ruler click and navigator zoom/pan; z/t/s/a region shortcuts; blur annotation obscuring for real (mosaic vs gaussian vs oval all visibly different); shortcut rebinding end to end (Add Zoom Z->X, persisted to `shortcuts.json`); persistence across quit+relaunch of every modifier and of composition settings. **Minor defects.** (a) **The diagnostic bundle carries no logs** - `recentLogs: []`, `mainProcessLogs: []`, `helperOutput` empty, `projectState: null`, while the live log at that moment held the STT failure and the export warnings; it cannot do the job it exists for. (b) **The blur inspector exposes no intensity or block-size control** (only Blur Type and Blur Shape) although the saved model carries `blurData: {intensity: 12, blockSize: 12}`, so that checklist line cannot be satisfied; there is also no freehand blur shape, only Rectangle and Oval. (c) **The HUD is stranded off-screen by a display-resolution change** - after switching to a smaller point size the HUD vanished, the global shortcut did nothing visible and the tray's Open item raised the editor instead; only quit+relaunch brought it back. The editor window is re-clamped correctly (`editor-window.json` -> `{"x":0,"y":30,"width":1152,"height":600}`), the HUD has no persisted position at all. Since the HUD is the only route back to the studio, a user who changes resolution or unplugs a display loses it until they restart. (d) **GIF timing at 15 FPS is 6 centiseconds per frame = 16.67 fps**, so a 62.40s programme becomes a 56.28s GIF (~10% fast); 15 and 30 fps are not representable in GIF's integer-centisecond delays, 20/25/50 are. **Non-defects, checked before reporting** (each of these looked like a failure first): the export going silent at exactly 30.1999s is the source's own silence mapped through the trim, not a mix bug; `timeline.speedRanges` staying `[]` is schema layout - speed regions live in `legacyEditor.speedRegions`; 58 "non monotonically increasing dts" lines from `ffmpeg -f null -` on the window capture are a null-muxer artifact on VFR input (ffprobe finds 0 non-increasing DTS across all 1161 packets); and an annotation that appeared not to render was white text on a white page. **Not covered**: AI chat and chat-driven editing (a provider IS connected - "OpenAI Compatible"), clip operations (drag-in, reorder, Edit Clip dialog, copy/paste), modifiers-anchored-to-clips, modifiers under a trim, imported audio and voice-over, undo/redo, the Background/Effects/Layout/Cursor facets, theme and locale switching, restart/cancel while recording, HUD minimize/close, webcam PiP and the AI camera background (**this Mac has no camera and no microphone**), a second display at a different scale factor, Windows and Linux. **Driver limits worth knowing**: `Escape` still never reaches the app (third run in a row - untestable, not failing); modifier+scroll (Ctrl/Shift+wheel zoom and pan) still does nothing, so the navigator was used instead; hover-revealed controls on transcript words and silence chips could not be reached at the inspector's fixed width. **Do not use `osascript` to drive other apps mid-pass** - it raised a macOS Automation dialog that is masked out of computer-use screenshots and then refused clicks with `"UserNotificationCenter" is not in the allowed applications` until the operator cleared it; the Displays settings pane also runs as its own process that `request_access` cannot resolve, so its dropdowns are unclickable. |
|      |             |          |           |       |
|      |             |          |           |       |
